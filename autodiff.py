...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        if node.matmul_attr_trans_A:
            valA = np.transpose(input_vals[0])
        else:
            valA = input_vals[0]
        if node.matmul_attr_trans_B:
            valB = np.transpose(input_vals[1])
        else:
            valB = input_vals[1]
        return np.matmul(valA, valB)
    ...
    def gradient(self, node, output_grad):
        ...
        """TODO: Your code here"""
        if node.matmul_attr_trans_A:
            gradB = matmul_op(node.inputs[0], output_grad)
        else:
            gradB = matmul_op(node.inputs[0], output_grad, True)
        if node.matmul_attr_trans_B:
            gradA = matmul_op(output_grad, node.inputs[1])
        else:
            gradA = matmul_op(output_grad, node.inputs[1], False, True)
        return [gradA, gradB]
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            ...
            for n in node.inputs:
                inputs.append(node_to_val_map[n])
            if type(node.op) != PlaceholderOp:
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        if node not in node_to_output_grads_list:
            node_to_output_grads_list[node] = None
            continue
        ...
        if grads:
            for i, n in enumerate(node.inputs):
                ...
                node_to_output_grads_list[n].append(grads[i])
